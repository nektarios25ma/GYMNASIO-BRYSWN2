//ΤΟ ΠΡΟΓΡΑΜΜΑ ΤΗΣ ΜΠΑΡΑΣ ΤΟΥ ΝΟΣΟΚΟΜΕΙΟΥ , ΤΗΣ ΟΘΟΝΗΣ LCD ΚΑΙ ΤΗΣ ΣΤΑΔΙΑΚΗΣ ΦΩΤΕΙΝΟΤΗΤΑΣ ΤΗΣ ΠΟΛΗΣ(ΟΣΟ ΠΕΡΙΣΣΟΤΕΡΟ ΣΚΟΤΑΔΙ ΤΟΣΟ ΠΙΟ ΠΟΛΥ ΦΩΤΙΖΟΥΝ) KAI!.....
// I2C LCD screen demo
...
LDR=analogRead(A1);
myservo.write(180); // Γύρισε τον άξονα του servo στην τιμή 0 //-----------------------------------------------------------------------------------------------
...
{ 
  LDR=analogRead(A1); 
  value = analogRead(A2); //BUTTON1=869  ,BUTTON2=815 , BUTTON3=726 ,BUTTON4=543, BUTTON5=3 OR 4
  Serial.println("value=");
  Serial.println(value);  
if (value > 830 && value < 1000) {
    playEleventh();
    //Serial.println("Button1");
  } else if (value > 765 && value < 830) {
    playTenth();
    //Serial.println("Button2");
  } else if (value > 660 && value < 765) {
    playEighth();
    //Serial.println("Button3");
  } else if (value > 470 && value < 660) {
    playNinth();
...
// Serial.println(input);  
 brightness = map(LDR,129,893,0,255);  //ΣΤΑΔΙΑΚΟΣ ΦΩΤΙΣΜΟΣ ΠΟΛΗΣ ΑΝΤΙΣΤΡΟΦΩΣ ΑΝΑΛΟΓΗ ΤΗΣ ΕΞΩΤΕΡΙΚΗΣ ΦΩΤΕΙΝΟΤΗΤΑΣ
...
playSixth();
playSeventh();
//ΕΑΝ Η ΘΕΡΜΟΚΡΑΣΙΑ ΕΙΝΑΙ <14 C ΓΡΑΨΕ " COLD ,PROTECT!!!" ΣΤΗΝ ΟΘΟΝΗ LCD ΚΑΙ ΠΑΙΞΕ ΤΟ ΑΝΤΙΣΤΟΙΧΟ ΚΟΜΜΑΤΙ ΑΠΟ DFPLAYER-SD CARD  
if (temp<14.00)
{
  lcd.setCursor(0,1);
...
//ΕΑΝ Η ΘΕΡΜΟΚΡΑΣΙΑ ΕΙΝΑΙ >28 C ΓΡΑΨΕ "HOT CLIMATE,PROTECT" ΣΤΗΝ ΟΘΟΝΗ ΚΑΙ ΠΑΙΞΕ ΤΟ ΑΝΤΙΣΤΟΙΧΟ ΚΟΜΜΑΤΙ ΑΠΟ DFPLAYER-SD CARD 
if (temp>29.00)
{
  lcd.setCursor(0,1);
...
//ΕΑΝ ΔΕΝ ΑΝΙΧΝΕΥΤΕΙ ΥΠΕΡΗΧΟΣ
if (( TP_init()<=10) || (TP_init()>29930)) {
  value = analogRead(A2); 
start_time=millis(); //ΑΡΧΙΖΩ ΤΟ ΧΡΟΝΟΜΕΤΡΟ 
while (millis() - start_time < 2500) { // Εφόσον δεν πέρασαν 2,5 δευτ κάνε:
  myservo.write(180); // Γύρισε τον άξονα του servo στην τιμή 0-κλεισε την μπάρα  //-------------------------------------------------------------------------
delay(20);
LDR=analogRead(A1); 
//Read data and store it to variables hum and temp,ΔΙΑΒΑΖΩ ΜΕΤΡΗΣΕΙΣ ΚΑΙ ΤΙΣ ΑΠΟΘΗΚΕΥΩ ΣΤΙΣ ΜΕΤΑΒΛΗΤΕΣ hum and temp
    hum = dht.readHumidity();
...
void playFifth() //κατευθυνθητε σε θερμαινομενους χωρους
{
...
void playSixth() //ΠΡΟΣΟΧΗ ερχεται ασθενοσφορο...
{
...
void playSeventh() //ΠΡΟΣΟΧΗ ερχεται ασθενοσφορο...
{
...
void playEighth() //ΟΛΗ Η ΕΞΗΓΗΣΗ ΠΑΤΕΝΤΑΣ FINLAND
{
...
void playNinth() //ΟΛΗ Η ΕΞΗΓΗΣΗ ΠΑΤΕΝΤΑΣ ΑΓΓΛΙΚΑ
{
...
void playTenth() //ΟΛΗ Η ΕΞΗΓΗΣΗ lie detector ΑΓΓΛΙΚΑ
{
...
void playEleventh() //ΟΛΗ Η ΕΞΗΓΗΣΗ lie detector finland
{
...
{
  execute_CMD(0x29, 0, volume); // Set the volume (0x00~0x30)
  delay(1000);
}
